...
export default function ProfilePage() {
      ...
      if (raw) {
        ...
        const email =
          typeof window !== "undefined"
            ? sessionStorage.getItem("kaa_email") || u?.email
            : u?.email;
        const phone =
          typeof window !== "undefined"
            ? sessionStorage.getItem("kaa_phone") || u?.phone_number
            : u?.phone_number;
        setUser({
          firstName,
          lastName,
  ...
  async function loadBalances() {
    ...
    try {
      ...
      const credit =
        payload?.data?.credit_balance ?? payload?.credit_balance ?? null;
      const wallet =
        payload?.data?.wallet_balance ?? payload?.wallet_balance ?? null;
  ...
  async function loadHistory(page = 1) {
    ...
    try {
      // getCreditsHistory should accept a page param or default to first page.
      // Adjust call if your service expects a different signature.
      ...
      const pageData: CreditsPage = {
        ...
        next_page_url: payload?.data?.next_page_url ?? null,
  ...
  async function onConvertSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const summary = {
        success: true,
        wallet_amount_deducted:
          body?.wallet_amount_deducted ?? body?.walletAmountDeducted ?? null,
        credits_added: body?.credits_added ?? body?.creditsAdded ?? null,
        new_wallet_balance:
          body?.new_wallet_balance ?? body?.newWalletBalance ?? null,
        new_credit_balance:
          body?.new_credit_balance ?? body?.newCreditBalance ?? null,
        conversion_rate: body?.conversion_rate ?? body?.conversionRate ?? null,
      };
      setConvertResponse(summary);
      await loadBalances();
      await loadHistory(1); // refresh history after conversion
    } catch (err: any) {
  ...
  const isDebitType = (t: string | undefined) => {
    ...
    return (
      low.includes("deduct") || low.includes("debit") || low.includes("charge")
    );
  };

  const handleLoadMore = () => {
    if (historyMeta?.next_page_url) {
      // naive next page calculation
      const nextPage = (historyMeta.current_page || 1) + 1;
      ...
      <main className="container mx-auto px-4 lg:px-8 py-8 space-y-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            ...
            {user.phone && (
              <div className="text-sm text-foreground">Phone: {user.phone}</div>
            )}
          </div>
        </div>
            ...
            <>
              <div className="text-foreground">
                ...
                <span className="font-semibold">
                  {balances.credit == null ? "-" : fmtCurrency(balances.credit)}
                </span>
              </div>
              <div className="text-foreground">
                Wallet Balance:{" "}
                <span className="font-semibold">
                  {balances.wallet == null ? "-" : fmtCurrency(balances.wallet)}
                </span>
              </div>
            </>
          )}
            ...
            <div className="bg-card rounded-xl border shadow-sm p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                ...
                <div className="font-medium">
                  {fmtCurrency(convertResponse.wallet_amount_deducted)}
                </div>
                <div className="text-muted-foreground">Credits Added</div>
                <div className="font-medium">
                  {fmtCurrency(convertResponse.credits_added)}
                </div>
                <div className="text-muted-foreground">New Wallet Balance</div>
                <div className="font-medium">
                  {fmtCurrency(convertResponse.new_wallet_balance)}
                </div>
                <div className="text-muted-foreground">New Credit Balance</div>
                <div className="font-medium">
                  {fmtCurrency(convertResponse.new_credit_balance)}
                </div>
                <div className="text-muted-foreground">Conversion Rate</div>
                <div className="font-medium">
                  {convertResponse.conversion_rate == null
                    ? "-"
                    : new Intl.NumberFormat("en-US", { maximumFractionDigits: 4 }).format(
                        Number(convertResponse.conversion_rate)
                      )}
                </div>
              </div>
            </div>
        ...
        <section className="space-y-3">
                ...
                {history.length === 0 && !loadingHistory ? (
                  ...
                  history.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td className="px-4 py-3 text-sm">
                        {fmtDate(tx.updated_at ?? tx.created_at)}
                      </td>
          ...
          {/* load more */}
          {historyMeta?.next_page_url && (
            <div className="text-center">
              <Button onClick={handleLoadMore} disabled={loadingHistory}>
            ...
            <div className="bg-white rounded-xl w-full max-w-2xl p-6 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    {fmtDate(selectedTx.updated_at ?? selectedTx.created_at)}
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-medium">
                    {fmtCurrency(selectedTx.balance_before)}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">
                  ...
                  <div className="font-medium">
                    {fmtCurrency(selectedTx.balance_after)}
                  </div>
                </div>
              </div>
              ...
              {selectedTx.property && (
                <div className="mb-4">
                  ...
                  <div className="text-sm font-medium">
                    <a
                      href={`/property/${selectedTx.property.id}`}
                      className="text-primary hover:underline"
                    >
                      {selectedTx.property.title ??
                        `Property #${selectedTx.property.id}`}
                    </a>
                  </div>
                </div>
              )}

              {/* removed raw JSON block for cleaner UI */}
